...
export default function ProductAddReview({
  u_id,
  product,
  username,
}: {
  ...
  function handleFormSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!reviewRef.current?.value || !stars || !u_id || !username || !product)
      return;
  ...
  async function submitReview(userReview: tReview) {
    try {
      const response = await fetch(
        "https://backend-production-e988.up.railway.app/api/reviews/addreview",
        {
          method: "POST",
          credentials: "include",
          mode: "cors",
  ...
  return (
    <div className="flex flex-col items-center gap-2 w-full md:w-[50%] p-2">
      ...
      <div
        onMouseLeave={() => starContainerOnMouseLeave()}
        className="flex justify-center gap-2">
        {[1, 2, 3, 4, 5].map((n, i) => {
          return n <= (hoverStars || Math.abs(hoverStars - stars)) ? (
            <AiFillStar
              onMouseLeave={() => fillStarOnMouseLeave()}
              onMouseEnter={() => fillStarOnMouseEnter(i)}
              key={i}
              size={30}
              onClick={() => setStars(i + 1)}
              className="fill-amber-300 hover:cursor-pointer"
            />
          ) : (
            <AiOutlineStar
      ...
      <form
        className="self-center flex flex-col items-center justify-center gap-4 w-full"
        onSubmit={handleFormSubmit}>
        <textarea
          ref={reviewRef}
          className="border-2 border-[#292F36] rounded-md p-2 w-full md:w-[80%]"
          name="review"
          id="review"
          cols={30}
          rows={5}></textarea>
