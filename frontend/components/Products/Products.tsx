...
export default function Products() {
  ...
  const [starFilter, setStarFilter] = useState<boolean[]>(() =>
    setInitalStarFilter(currentStars)
  );
  ...
  function onChangeStarFilter(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 0; i < updatedStarFilter.length; i++) {
      if (updatedStarFilter[i] === true)
        starsQueryString = starsQueryString.concat(`${i + 1}`);
    }
  ...
  const productsByStars = useCallback(
    (array: tProduct[]) => {
      return array
        .filter((product) => {
  ...
  const productsByPrice = useCallback(
    (array: tProduct[]) => {
      return array
        .filter((product) => {
          const productPrice = product.price_cent * 0.01;

          if (minPriceFilter && maxPriceFilter) {
            return (
              productPrice >= Number(minPriceFilter) &&
              productPrice <= Number(maxPriceFilter)
            );
          }
          if (minPriceFilter && !maxPriceFilter) {
  ...
  const filterProductsCurrentPage = useMemo(
    () => filteredProducts.slice(pagePointerStart, pagePointerEnd),
    [filteredProducts, pagePointerStart, pagePointerEnd]
  );

  const numPages = useMemo(
    () => Math.ceil(filteredProducts.length / pageRange),
      ...
      <div className="flex flex-col sm:items-stretch sm:flex-row flex-1 w-full">
        ...
        <div className="productsContainer flex-1 w-full md:w-fit">
          {filterProductsCurrentPage
            ? filterProductsCurrentPage?.map((product) => (
                <Product
                  key={product.p_id}
                  product={product}
                />
