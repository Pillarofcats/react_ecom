...
export const setUserInfoOnLoad = createAsyncThunk(
  "setUserInfoOnLoad",
  async function () {
    try {
      //localhost:5000
      const response = await fetch(
        "https://backend-production-e988.up.railway.app/api/users/auth",
        {
          method: "GET",
          credentials: "include",
          mode: "cors",
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(
      ...
      builder.addCase(
        getUserPurchases.fulfilled,
        (state, action: PayloadAction<tPurchase[]>) => {
