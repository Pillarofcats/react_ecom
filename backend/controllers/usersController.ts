...
const userSignUp = async function(req:Request, res:Response) {
  ...
  try {
    ...
    if(selectUser.rows.length !== 0) return res.status(200).json({message:"User already exists", status:"err"})
    if(password.length < 8) return res.status(200).json({message:"Password must be 8 chars", status:"err"})
...
const userSignIn = async function(req:Request, res:Response) {
  ...
  try {
    ...
    if(signin.rows.length === 0) return res.status(401).json({message:"User doesn't exist", status:"err"})

    const validPassword = await bcrypt.compare(password, hashedPassword)

    if(!validPassword) return res.status(401).json({message:"Incorrect password", status:"err"})
