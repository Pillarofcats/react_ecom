...
const singleProduct = async function (req: Request, res: Response) {
  ...
  try {
    const products = await dbQuery(
      "SELECT * FROM ecom.all_products WHERE p_id = $1",
      [pid]
    );
...
const purchase = async function (req: Request, res: Response) {
  ...
  if (u_id) {
    ...
    try {
      for (const index in queryValues) {
        await dbQuery(
          "INSERT INTO ecom.user_purchases (u_id, p_id, stars, title, price_cent, qty_purchased) VALUES($1, $2, $3, $4, $5, $6)",
          queryValues[index]
        );
      }
    } catch (error) {
  ...
  try {
    for (const index in queryValues) {
      await dbQuery(
        `UPDATE ecom.all_products SET quantity = $1 WHERE p_id = $2`,
        queryValues[index]
      );
    }
